job "agreement_maker" {
  ...
  group "agreement-processor" {
    ...
    task "processor" {
      ...
      resources {
        memory = 15000 # Higher memory for large raster processing. This should be DASK_CLUST_MAX_MEM + a bit more. Setting this to a bit higher than the max memory usage seen from agreement job while processing ripple data
      }
